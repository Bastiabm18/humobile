// components/calendar/BlockDateModal.tsx
'use client';

import { useState } from 'react';
import { format } from 'date-fns';
import { HiX, HiLockClosed, HiCalendar } from 'react-icons/hi';
import { createDateBlock } from '../actions/actions';
...
export default function BlockDateModal({ open, onClose, profile, initialDate }: BlockDateModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim() || !reason.trim()) {
      setModalState({
        isOpen: true,
        mensaje: 'El título y el motivo son obligatorios',
        esExito: false,
      });
      return;
    }
    ...
    if (!startDateTime) {
      setModalState({
        isOpen: true,
        mensaje: 'La fecha/hora de inicio es obligatoria',
        esExito: false,
      });
      return;
    }
    ...
    setModalState({
      ...
      mensaje: result.success
        ...
        : result.error || 'Error desconocido al crear el bloqueo',
      esExito: result.success,
    });

    if (result.success) {
      // Puedes cerrar automáticamente después de 2 segundos si quieres
      setTimeout(() => {
        onClose();
      }, 2000);
    }
