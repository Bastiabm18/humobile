...
interface ModalSeleccionarParticipantesProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ModalSeleccionarParticipantes({
  isOpen,
  onClose,
  perfilesDisponibles,
  participantesActuales,
  onAgregar
}: ModalSeleccionarParticipantesProps) {
        ...
        <div className="p-4 border-b border-neutral-700 space-y-3">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${!filtroTipo ? 'bg-green-600 text-white' : 'bg-neutral-700 text-gray-300 hover:bg-neutral-600'}`}
            >
              Todos
            </button>
            <button 
              onClick={() => setFiltroTipo('artista')}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${filtroTipo === 'artista' ? 'bg-green-600 text-white' : 'bg-neutral-700 text-gray-300 hover:bg-neutral-600'}`}
            >
              Artistas
            </button>
            <button 
              onClick={() => setFiltroTipo('banda')}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${filtroTipo === 'banda' ? 'bg-green-600 text-white' : 'bg-neutral-700 text-gray-300 hover:bg-neutral-600'}`}
            >
              Bandas
            </button>
