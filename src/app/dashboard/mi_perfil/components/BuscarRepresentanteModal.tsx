...
import { 
  enviarSolicitudRepresentante, getRepresentantesPaginadosInvitacion, getMisRepresentantesActivos, abandonarRepresentanteAction } from '../actions/actions'
import { format, addDays } from 'date-fns';

interface Props {
  id_perfil: string;           // El ID del perfil que está solicitando (artista/banda)
  isOpen: boolean;
  onClose: () => void;
}

export default function BuscarRepresentanteModal({ id_perfil, isOpen, onClose }: Props) {
  // Estados de Navegación
  const [view, setView] = useState<'explorar' | 'mis_representantes'>('explorar');
  ...
  // Estados de Datos (Mis Representantes)
  const [misRepresentantes, setMisRepresentantes] = useState<any[]>([]);
  ...
  // Datos del formulario (solo para enviar)
  const [mensaje, setMensaje] = useState('');

  const observer = useRef<IntersectionObserver | null>(null);

  // Infinite Scroll Ref (para explorar)
  ...
  // Carga inicial y búsqueda (Explorar)
  useEffect(() => {
    ...
    const fetchInitial = async () => {
      ...
      setPage(0);
      try {
        const { data, hasMore: more } = await getRepresentantesPaginadosInvitacion(
          id_perfil,
          0,
          10,
          busqueda
        ...
        console.error(e);
        setStatus({ type: 'error', msg: 'Error al cargar representantes' });
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [busqueda, isOpen, id_perfil, view]);

  // Carga de Mis Representantes
  useEffect(() => {
    if (isOpen && view === 'mis_representantes') {
      const loadMisRepresentantes = async () => {
        setLoading(true);
        try {
          const { data } = await getMisRepresentantesActivos(id_perfil);
          setMisRepresentantes(data || []);
        } catch (e) {
          console.error(e);
          setStatus({ type: 'error', msg: 'Error al cargar tus representantes' });
        } finally {
          setLoading(false);
        }
      };
      loadMisRepresentantes();
    }
  }, [isOpen, view, id_perfil]);
  ...
  useEffect(() => {
    ...
    const fetchMore = async () => {
      ...
      try {
        ...
        setRepresentantes(prev => [...prev, ...(data || [])]);
        setHasMore(more);
      } catch (e) {
        console.error(e);
      } finally {
    ...
    fetchMore();
  }, [page, isOpen, id_perfil, busqueda, view]);

  const ejecutarEnvio = async (idRepresentante: string) => {
    setEnviandoId(idRepresentante);
    setConfirmarId(null);
    try {
      await enviarSolicitudRepresentante({
        id_perfil,
      ...
      setTimeout(() => {
        setRepresentantes(prev => prev.filter(r => r.id_perfil !== idRepresentante));
        setStatus(null);
      }, 2000);
    } catch (error) {
  ...
  const handleAbandonar = async (idRepresentado: string) => {
    ...
    try {
      await abandonarRepresentanteAction(idRepresentado);
      setStatus({ type: 'success', msg: 'Has dejado de estar representado' });
      setMisRepresentantes(prev => prev.filter(r => r.id_representado !== idRepresentado));
      setConfirmarId(null);
      ...
      >
        ...
        <div className="p-6 border-b border-neutral-800 bg-neutral-900/50">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <FaUserTie className="text-blue-500" /> Mis Representantes
              </h2>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex p-1 bg-neutral-950 rounded-2xl border border-neutral-800">
            ...
            <button
              onClick={() => setView('mis_representantes')}
              className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-all ${
                view === 'mis_representantes'
                  ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20'
                  : 'text-neutral-500 hover:text-white'
              }`}
            >
          ...
          {loading ? (
            ...
            <>
              {representantes.map((rep, index) => {
                const isConfirming = confirmarId === rep.id_perfil;
                return (
                  <motion.div
                    layout
                    key={rep.id_perfil}
                    ref={index === representantes.length - 1 ? lastElementRef : null}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-2xl bg-neutral-800 border border-neutral-700 overflow-hidden">
                        {rep.imagen_url ? (
                          <img src={rep.imagen_url} className="w-full h-full object-cover" />
                        ) : (
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-white text-lg">{rep.nombre}</h3>
                          {rep.categoria && (
                        ...
                        <p className="text-xs text-neutral-500">{rep.nombre_comuna || 'Ubicación oculta'}</p>
                      </div>
                    </div>
                      ...
                      <AnimatePresence mode="wait">
                        {!isConfirming ? (
                          <motion.button
                            ...
                            exit={{ opacity: 0 }}
                            onClick={() => {
                              setConfirmarId(rep.id_perfil);
                              // Opcional: podrías abrir un pequeño campo para mensaje
                            }}
            ...
            // Vista "Mis Representantes"
            <>
              {misRepresentantes.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-2xl bg-neutral-800 border border-neutral-700 overflow-hidden">
                        {item.imagen_url_representante ? (
                          <img
                            src={item.imagen_url_representante}
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-white text-lg">{item.nombre_representante}</h3>
                        </div>
              ...
              {misRepresentantes.length === 0 && (
                <div className="py-20 text-center flex flex-col items-center gap-4">
                  ...
                  <p className="text-neutral-500 italic max-w-[200px]">
                    Aún no tienes representantes asociados.
                  </p>
                </div>
              )}
            </>
